...
public class AutoCrystalFeature extends Feature implements UpdateListener, KeyPressListener
{
	...
	public void onUpdate()
	{
		...
		else if (!MC.world.getDimension().respawnAnchorWorks() && HoleUtils.isSurrounded(target.getBlockPos()) && (!BlockUtils.hasBlock(target.getBlockPos().up(2)) || BlockUtils.isBlock(Blocks.RESPAWN_ANCHOR, target.getBlockPos().up(2))) && InventoryUtils.hasItemInHotbar(item -> item == Items.RESPAWN_ANCHOR) && InventoryUtils.hasItemInHotbar(item -> item == Items.GLOWSTONE))
		{
			killAura.setEnabled(false);
			crystalAura.setEnabled(false);
			autoCity.setEnabled(false);
